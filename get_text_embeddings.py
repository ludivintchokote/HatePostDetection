...
def validation(dl,model):
    ...
    for i, data in enumerate(dl):
        ...
        with torch.no_grad():
            predictions, merged, image_shifted , text_shifted = model(data['image'],data['text'], data['slabel'])
            predictions_softmax = nn.Softmax(dim=1)(predictions)
#             print(text_shifted.shape)
#             print(predictions_softmax,data['slabel'])
            ...
            text_embeddings.extend(text_shifted.cpu().detach().numpy().tolist())
            single_labels.extend(data['slabel'])
            img_names.extend(data['img_info'])
    text_embeddings = np.array(text_embeddings)
#     print(text_embeddings.shape)
#     print(img_names[:20])
    return fin_targets, fin_outputs, single_labels, img_names, text_embeddings
...
def main():
    ...
    datasets = ['fb','multioff','mami','harmeme']
...
#     img_attacks = [None,imaugs.OverlayStripes(),imaugs.RandomNoise(var=10),imaugs.RandomNoise(var=1000),
#                    imaugs.ShufflePixels(1),imaugs.ShufflePixels(0.1)]
    attacks = ['original_text','ocr','spread_1','spread_3','newsprint','s&p','s&p_0.4','blur_text_5','s&p_text_0.2']
#     attacks = ['original_text','ocr','spread_1']
    dict_zero_shot = {}
    dataset_name = 'mami'
    dict_text_adv = {}
    dict_adv = {}
...
#         model.load_state_dict(torch.load('saved/'+dataset_name+'.pth'))
    ...
    #         for sub_data in datasets:
        ...
        targets, outputs, slabels, img_names, temb = validation(test_dataloader,model)
        f1_score_macro = f1_score(targets, outputs, average='macro')
        accuracy = accuracy_score(targets, outputs)
        print ("Final F1 score on test set: ", dataset_name, f1_score_macro)
        print("Final Accuracy on test set: ", dataset_name, accuracy)
        all_embeds.extend(temb.tolist())
#         dict_results[sub_data] = f1_score_macro
#         dict_zero_shot[dataset_name] = dict_results
        dict_adv[attacks[i]]=f1_score_macro
    print(dict_text_adv,dict_adv)
    all_embeds = np.array(all_embeds).reshape(len(attacks),1000,768)
    print(all_embeds.shape)
    np.save('text_embs.npy',all_embeds)
